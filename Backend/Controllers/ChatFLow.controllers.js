...
exports.createChatFlow = async (req, res) => {
  try {
    ...
    res.status(500).send("Error at creating chat flow");
  }
};
...
exports.getAllChatFlows = async (req, res) => {
    try {
        const chatFlows = await ChatFlow.find().populate("creator","-password");
        res.status(200).json({ chatFlows });
    } catch (error) {
        console.log(error.message);
        res.status(500).send("Error at fetching chat flows");
    }
};
